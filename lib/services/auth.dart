...
class AuthServices {
  ...
  UserUid userFromFirebaseUser(User user) {
  ...
  Future<void> login(
      wholePhoneNumber, Function codeInvalid, String userType) async {
    ...
    try {
      ...
      if (userType == 'Carrier') {
        await DatabaseService(userPhoneNumber: wholePhoneNumber)
            .newCarrierUserData('', 'New member', uid.toString());
        print('Carrier written to database');
      }
      if (userType == 'Lounge') {
        await DatabaseService(userPhoneNumber: wholePhoneNumber)
            .newLoungeUserData(uid.toString());
        print('Lounge User written to database');
      }
    } catch (e) {
...
// }
  ...
  void submitOTP(
      otpCode, wholePhoneNumber, Function codeInvalid, String userType) {
    ...
    login(wholePhoneNumber, codeInvalid, userType);
  }
